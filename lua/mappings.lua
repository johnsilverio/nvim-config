...
wk.register({
	...
	-- Mapeamento da tecla líder para o Neotree (indentação correta)
	["<leader>n"] = {
	  name = "[NEOTREE]",  -- Nome do mapeamento exibido no popup which-key
	  "<cmd>Neotree toggle<cr>",
	  "[NEOTREE] Alternar exibição da árvore de arquivos" -- Descrição do mapeamento no popup which-key
	},
...
-- Mover linhas para cima ou para baixo com Alt+j ou Alt+k
vim.keymap.set('n', '<M-j>', ':m .+1<CR>==', { noremap = true, silent = true })
vim.keymap.set('n', '<M-k>', ':m .-2<CR>==', { noremap = true, silent = true })
vim.keymap.set('v', '<M-j>', ":m '>+1<CR>`<my`>gv=gv", { noremap = true, silent = true })
vim.keymap.set('v', '<M-k>', ":m '<-2<CR>`>my`<gv=gv", { noremap = true, silent = true })

-- Mover blocos de texto para cima ou para baixo com <count>Alt+j ou <count>Alt+k
vim.keymap.set('x', '<M-j>', ":m '>+1<CR>`<my`>gv=gv", { noremap = true, silent = true })
vim.keymap.set('x', '<M-k>', ":m '<-2<CR>`>my`<gv=gv", { noremap = true, silent = true })

-- Mover o cursor para a esquerda no modo de inserção com Alt + h
vim.keymap.set('i', '<M-h>', '<C-o>h')
-- Mover o cursor para baixo no modo de inserção com Alt + j
vim.keymap.set('i', '<M-j>', '<C-o>j')
-- Mover o cursor para cima no modo de inserção com Alt + k
vim.keymap.set('i', '<M-k>', '<C-o>k')
-- Mover o cursor para a direita no modo de inserção com Alt + l
